class ProviderMerchant < Merchant
  enum :source, { plaid: "plaid", simplefin: "simplefin", lunchflow: "lunchflow", akahu: "akahu", up: "up", synth: "synth", ai: "ai", enable_banking: "enable_banking", coinstats: "coinstats", mercury: "mercury", brex: "brex", indexa_capital: "indexa_capital", sophtron: "sophtron", willbudget: "willbudget" }
  ...
  def generate_logo_url_from_website!
    return if logo_url.present? && !logo_url.include?("fallback/lettermark") && !logo_url.include?("fallback/404")
    if website_url.present? && Setting.brand_fetch_client_id.present?
      domain = extract_domain(website_url)
      size = Setting.brand_fetch_logo_size
      update!(logo_url: "https://cdn.brandfetch.io/#{domain}/icon/fallback/404/w/#{size}/h/#{size}?c=#{Setting.brand_fetch_client_id}")
